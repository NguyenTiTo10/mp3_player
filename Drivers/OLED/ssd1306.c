...
char ssd1306_WriteChar(char ch, SSD1306_Font_t Font, SSD1306_COLOR color, uint8_t flag) {
    uint32_t i, b, j;
    ...
    if(flag){
        if (90 < (SSD1306.CurrentX + Font.width) ||
...
char ssd1306_WriteString(char* str, SSD1306_Font_t Font, SSD1306_COLOR color) {
    while (*str) {
        if (ssd1306_WriteChar(*str, Font, color, 0) != *str) { // modified
...
void ssd1306_ScrollString(char* str, uint8_t x, uint8_t y, SSD1306_Font_t Font, SSD1306_COLOR color, uint8_t flag)
{
    int len = strlen(str);
    char buffer[128]; // Fixed-size buffer

    if (len > 118) { // Check if string is too long for buffer
        return; // If so, return without doing anything
    }

    strcpy(buffer, str); // Copy original string to buffer
    strcat(buffer, "     "); // Add 5 spaces at the end

    while (1) { // Infinite loop
        for (int i = 0; i < len + 5; i++) {
            //ssd1306_Fill(Black); // Clear the screen
            ssd1306_SetCursor(x, y); // Reset cursor to the beginning
            for (int j = 0; j < 21 && buffer[i + j] != '\0'; j++) { // Write up to 21 characters
                ssd1306_WriteChar(buffer[i + j], Font, color, flag);
            }
            ssd1306_UpdateScreen(); // Update the screen
            HAL_Delay(200); // Delay for the scrolling speed
        }
    }
}
...
void ssd1306_DrawArrow(uint8_t x1, uint8_t y1, uint8_t x2, uint8_t y2, uint8_t gap, SSD1306_COLOR color)
{
	ssd1306_Line(x1, y1, x2, y2, color);
	ssd1306_Line(x1, y1 + gap, x2, y2, color);
	return;
}
